...
RUN --mount=type=cache,target=/go/pkg/mod --mount=type=cache,target=/root/.cache/go-build \
    ...
    CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -trimpath -ldflags='-s -w' -o /out/tracklume-admin ./cmd/admin && \
    CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -trimpath -ldflags='-s -w' -o /out/tracklume-seed ./cmd/seed
...
COPY --from=build /out/tracklume-admin /app/tracklume-admin
COPY --from=build /out/tracklume-seed /app/tracklume-seed
